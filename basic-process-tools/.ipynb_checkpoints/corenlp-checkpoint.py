...
class StanfordCoreNLP:
    def __init__(self, path_or_host, port=None, memory='4g', lang='en', timeout=1500, quiet=True,
        ...
        if path_or_host.startswith('http'):
            ...
            switcher = {
                'hu': 'stanford-corenlp-[0-9].[0-9].[0-9]-models-hungarian.jar',
                'it': 'stanford-corenlp-[0-9].[0-9].[0-9]-models-italian.jar',
            ...
            jars = {
                'hu': 'stanford-hungarian-corenlp-yyyy-MM-dd-models.jar',
                'it': 'stanford-italian-corenlp-yyyy-MM-dd-models.jar',
    ...
    def sent_tokenize(self, sentence, span=False):
        r_dict = self._request(self.url, 'ssplit,tokenize', sentence)
        sents_tokens = []
        for sentence in r_dict['sentences']:
            sents_tokens.append([token['originalText'] for token in sentence['tokens']])
        sentences = [" ".join(tokens) for tokens in sents_tokens]        # Whether return token span
        return sentences 
    
    def sent_word_tokenize(self, sentence, span=False):
        r_dict = self._request(self.url, 'ssplit,tokenize', sentence)
        sents_tokens = []
        for sentence in r_dict['sentences']:
            sents_tokens.append([token['originalText'] for token in sentence['tokens']])
    ...
    def _check_language(self, lang):
        if lang not in ['en', 'zh', 'ar', 'fr', 'de', 'es', 'hu', 'it']:
            raise ValueError('lang=' + self.lang + ' not supported. Use English(en), Chinese(zh), Arabic(ar), '
                                                   'French(fr), German(de), Spanish(es), Hungarian(hu), Italian(it).')